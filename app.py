...
def get_rep_count():
    ...
    bicep_count = reps['measure'].get('bicep_curls',0)
    pushup_count = reps['measure'].get('pushup_count',0)
...
def clear_all_files():
    files = ['gymstat.csv', 'plank.csv', 'tpose.csv', 'treepose.csv', 'warrior.csv']
    for file in files:
        if file == 'gymstat.csv':
            with open(f'data/{file}', 'w') as f:
                write = csv.writer(f)
                write.writerow(['Exercise', 'measure'])
        else:
            with open(f'data/{file}', 'w') as f:
                write = csv.writer(f)
                write.writerow(['pose','timestamp'])
...
def get_reports():
    ...
    total_calories = sum([calorie for calorie in calorie_report.values() if calorie > 0])
    return render_template('final_report.html', total_calories=round(total_calories), body_weight = weight ,clorie_dct=calorie_report,
                           reps_dct=reps_dct)
